...
function generateAutoHashtags(contentType = "meme", content = "") {
  ...
  if (content) {
    contentWords = content
      ...
      .filter(
        (word) =>
          !["this", "that", "when", "what", "where", "which", "with"].includes(
            word,
          ),
      );
  }
...
function validateHashtags(list, { min = 0, max = 30 } = {}) {
  if (!Array.isArray(list)) {
    return { valid: false, errors: ["Hashtags must be an array"] };
  }
  const errors = [];
  if (list.length < min) {
    errors.push(`At least ${min} hashtags required`);
  }
  if (list.length > max) {
    errors.push(`No more than ${max} hashtags allowed`);
  }
