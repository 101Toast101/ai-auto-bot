...
const validateSettings = (obj) => {
  ...
  if ('hashtagMode' in obj) {
    ...
    if (!validModes.includes(String(obj.hashtagMode))) {
      errors.push(`Invalid hashtagMode. Must be one of: ${validModes.join(', ')}`);
    }
  }
  if ('memeMode' in obj) {
    const validMemeModes = ['template', 'ai-generate', 'ai-edit', 'ai-variations'];
  ...
  const encryptedFields = ['openaiApiKey', 'runwayApiKey', 'instagramToken',
                          'tiktokToken', 'youtubeToken', 'twitterToken'];
...
const validateScheduledPosts = (obj) => {
  ...
  if (duplicates.length > 0) {
    errors.push(`Duplicate post IDs found: ${[...new Set(duplicates)].join(', ')}`);
  }
  return { valid: errors.length === 0, errors };
};
  ...
  obj.configs.forEach((config, index) => {
    ...
    if (!config.settings) {
      ...
      if (!settingsValidation.valid) {
        errors.push(`Config "${config.name}" at index ${index}: ${settingsValidation.errors.join('; ')}`);
      }
    }
  });
  return { valid: errors.length === 0, errors };
...
const validateActivityLog = (obj) => {
  ...
  arr.forEach((log, index) => {
    ...
    if ('status' in log && !['success', 'error', 'warning', 'info'].includes(log.status)) {
      errors.push(`${prefix} Invalid 'status' (must be success, error, warning, or info)`);
    }
  });
  return { valid: errors.length === 0, errors };
};
